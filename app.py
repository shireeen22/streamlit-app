...
AUTHENTICATION_API = "https://authentication-api-1-v2pt.onrender.com"
            ...
            if st.button("Create account"):
                if(full_name.strip()== ""
                   ...
                   password.strip()== ""):
                    ...
                    try:
                        response = requests.post(f"{AUTHENTICATION_API}/register",json={
                            "full_name":full_name,
                            "email":email,
                            "username":username,
                            "password":password})
        ...
        with tab2:
            ...
            if st.button("Login"):
                if username.strip()=="" or password.strip()=="":
                    ...
                    try:
                        response = requests.post(f"{AUTHENTICATION_API}/login",
                                             json={
                                                 "username":username,
                                                 "password":password})
                        data = response.json()
